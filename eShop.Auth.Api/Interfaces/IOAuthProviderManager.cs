...
public interface IOAuthProviderManager
{
    ...
    public ValueTask<Result> ConnectAsync(UserEntity user, OAuthProviderEntity provider, CancellationToken cancellationToken = default);
    public ValueTask<Result> DisconnectAsync(UserEntity user, OAuthProviderEntity provider, CancellationToken cancellationToken = default);
}