...
public static class DistributedApplicationExtensions
{
    extension(IDistributedApplicationBuilder builder)
    {
        ...
        {
            var options = builder.Configuration.GetSectionValue<MsSqlOptions>("MSSQL");
        ...
        {
            var options = builder.Configuration.GetSectionValue<PostgresOptions>("PostgreSQL");
            var password = builder.AddParameter("postgres-password", options.Password);
            var username = builder.AddParameter("postgres-username", options.Username);
            
            return builder.AddPostgres(options.Name, username, password, options.Port);
        }

        public IResourceBuilder<RedisResource> AddRedis()
        {
            var options = builder.Configuration.GetSectionValue<RedisOptions>("Redis");
        ...
        {
            var options = builder.Configuration.GetSectionValue<RabbitMqOptions>("RabbitMQ");
