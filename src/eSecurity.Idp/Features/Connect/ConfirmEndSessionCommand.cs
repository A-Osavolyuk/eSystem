...
public sealed record ConfirmEndSessionCommand : IRequest<Result>
{
    ...
    IHttpContextAccessor httpContextAccessor) : IRequestHandler<ConfirmEndSessionCommand, Result>
{
    ...
    public async Task<Result> Handle(ConfirmEndSessionCommand request, CancellationToken cancellationToken = default)
    {
        ...
        var endSessionRequest = await _endSessionManager.FindByIdAsync(request.RequestId, cancellationToken);
        if (endSessionRequest is null)
        {
            return Fallback(redirectUri, ErrorCode.InvalidRequest,
                "end_session_request_uri is invalid", request.State);
        }

        if (endSessionRequest.Status is not EndSessionStatus.Pending)
        {
            return Fallback(redirectUri, ErrorCode.InvalidRequest,
                "End session request is invalid. Please, try again later", request.State);
        }
        ...
        {
            if (string.IsNullOrEmpty(request.State) || !endSessionRequest.State.Equals(request.State))
            {
                return Fallback(redirectUri, ErrorCode.InvalidRequest,
