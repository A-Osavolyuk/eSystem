...
public record CreateSoftwareKeyCommand : IRequest<Result>
{
    ...
    [JsonPropertyName("response")] public PublicKeyCredentialCreationResponse? Response { get; set; }
}
...
public class CreateSoftwareKeyCommandHandler(
    ...
    IAttestationProcessor attestationProcessor,
    IOptions<CredentialOptions> options) : IRequestHandler<CreateSoftwareKeyCommand, Result>
{
    ...
    private readonly IAttestationProcessor _attestationProcessor = attestationProcessor;
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(savedChallenge))
        {
        ...
        var clientDataHash = SHA256.HashData(request.Response.Response.ClientDataJson);
        var attestationObject = AttestationObject.Parse(request.Response.Response.AttestationObject);
        var attestationProcessResult = await _attestationProcessor.ProcessAsync(attestationObject,
            clientDataHash, cancellationToken);

        if (!attestationProcessResult.Succeeded)
        {
            var error = attestationProcessResult.GetError();
            return Results.ClientError(ClientErrorCode.BadRequest, error);
        }

        var attestationVerificationResult = attestationProcessResult.GetValue();
        if (!attestationVerificationResult.IsValid)
        {
            return Results.ClientError(ClientErrorCode.BadRequest, new Error()
            {
        ...
        var softwareKey = new SoftwareKeyEntity
        {
            ...
            AuthenticatorId = new Guid(attestationObject.AuthenticationData.AaGuid),
            DeviceId = device.Id,
            DisplayName = request.DisplayName,
            Domain = clientData.Origin,
            CredentialId = attestationObject.AuthenticationData.CredentialId,
            PublicKey = attestationObject.AuthenticationData.CredentialPublicKey,
            SignCount = attestationObject.AuthenticationData.SignCount,
            Type = request.Response.Type,
            AttestationFormatType = attestationObject.FormatType,
            AttestationTrustType = attestationVerificationResult.TrustType
        };
