    ...
    IConfiguration configuration) : ApiService(configuration, pipe), ISecurityService
{
    ...
    public async ValueTask<HttpResponse> ResetRecoveryEmailAsync(ResetRecoveryEmailRequest request) => await ApiClient.SendAsync(
        new HttpRequest { Url = $"{Gateway}/api/v1/Security/recovery-email/reset", Method = HttpMethod.Post, Data = request },
        new HttpOptions { WithBearer = true, Type = DataType.Text });
    ...
    public async ValueTask<HttpResponse> VerifyRecoveryEmailAsync(VerifyRecoveryEmailRequest request) => await ApiClient.SendAsync(
        new HttpRequest { Url = $"{Gateway}/api/v1/Security/recovery-email/verify", Method = HttpMethod.Post, Data = request },
        new HttpOptions { WithBearer = true, Type = DataType.Text });

    public async ValueTask<HttpResponse> AddRecoveryEmailAsync(AddRecoveryEmailRequest request) => await ApiClient.SendAsync(
    ...
    public async ValueTask<HttpResponse> ChangeRecoveryEmailAsync(ChangeRecoveryEmailRequest request) => await ApiClient.SendAsync(
        new HttpRequest { Url = $"{Gateway}/api/v1/Security/recovery-email/change", Method = HttpMethod.Post, Data = request },
        new HttpOptions { WithBearer = true, Type = DataType.Text });
