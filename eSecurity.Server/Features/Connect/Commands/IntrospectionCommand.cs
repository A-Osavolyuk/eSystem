...
public class IntrospectionCommandHandler(
    ...
    IOptions<TokenOptions> tokenOptions) : IRequestHandler<IntrospectionCommand, Result>
{
    ...
    public async Task<Result> Handle(IntrospectionCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var principal = handler.ValidateToken(request.Request.Token, validationParameters, out var validatedToken);
